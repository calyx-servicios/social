...
class Mail(models.Model):
    ...
    def send_get_mail_cc(self, _partner=None):
        ...
        _logger.debug('mail ids?: %r ',self.ids)
        
        for mail_id in self.ids:
            mail = self.browse(mail_id)
            _logger.debug('mail subject: %r ',mail.subject)
            email_cc = []
            for partner in mail.recipient_ids:
                if partner.id != _partner.id:
                    if partner.email not in email_cc and partner.email != mail.email_to:
                        _logger.info('mail.recipient_ids partner?: %r ',partner.email)
                        email_cc.append(partner.email)
            _logger.info('send_get_mail_cc: %r ',email_cc)
            if mail.email_from not in email_cc and mail.email_from != mail.email_to:
    ...
    def _postprocess_sent_message(self, mail_sent=True):
        _logger.debug('postprocess_sent_message model?: %r ',self.model)
            ...
            try:
                ...
                for email in email_list[:1]:
                    if mail.email_from not in email.get('email_cc'):
                       _logger.info('must assign to cc list %r ',mail.email_from) 
                    else:
                        _logger.info('from is assigned to cc list %r ',mail.email_from)
